...
void main() {
  ...
  group('buildImage Widget', () {
    testWidgets('renders Image with basic properties', (WidgetTester tester) async {
      final ref = ImageRef.memory(Uint8List.fromList(const [1,2,3,4])); // Use memory for simplicity
    ...
    testWidgets('uses provided placeholder during loading', (WidgetTester tester) async {
      ...
      final placeholderWidget = SizedBox(key: placeholderKey, child: const Text('Custom Loading...'));
      ...
      final Image imageWidget = Image(
        image: const NetworkImage('http://example.com/fake.png'), // A provider that would trigger loading
      ...
      final Widget builtLoadingWidget = imageWidget.loadingBuilder!(
        ...
        const ImageChunkEvent(cumulativeBytesLoaded: 50, expectedTotalBytes: 100), // Simulate progress
      );
    ...
    testWidgets('uses default CircularProgressIndicator when no placeholder and loading', (WidgetTester tester) async {
    ...
    testWidgets('shows child when loading complete', (WidgetTester tester) async {
       final Image imageWidget = Image(
        image: const NetworkImage('http://example.com/fake.png'),
        loadingBuilder: (context, child, progress) { // Using buildImage's typical logic
          if (progress == null) return child;
    ...
    testWidgets('uses default error text when no errorWidget and failure (missing asset)', (tester) async {
    ...
    testWidgets('applies cacheWidth and cacheHeight via providerFor to Image widget', (WidgetTester tester) async {
